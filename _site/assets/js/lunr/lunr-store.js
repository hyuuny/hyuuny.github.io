var store = [{
        ...
        "title": "인프라 환경",
        "excerpt":"컨테이너 인프라 환경은 컨테이너를 중심으로 구성된 인프라 환경이다. 컨터이너(container)는 하나의 운영체제 커널에서 다른 프로세스에 영향을 받지 않고 독립적으로 실행되는 프로세스 상태를 의미한다. 이렇게 구현된 컨테이너느 가상화 상태에서 동작하는 프로세스보다 가볍고 빠르게 동작한다. 모놀리식 아키텍쳐 모놀리식 아키텍처는 하나의 큰 목적이 있는 서비스 또는 애플리케이션에 여러 기능이 통합돼 있는 구조를 의미한다. 모놀리식...","categories": ["Docker-Kubernetes"],
        "tags": ["2021년 10월"],
